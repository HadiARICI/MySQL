...
--2 sorgu ifadesini birleştirmeye yarar
...
--SORU1: musteriler tablosunda sehir'i Istanbul olan veriler ile yasi 
...
select urun_adi as urunler_yazarlar , musteri_id from siparisler where urun_adi like 'k%'
union
...
select id, sehir as sehirler_urunler from musteriler where yas<30
union
...
--tekrarlı ifade istiyorsak
--union all

select isim from musteriler where sehir like 'I%'
union all
select isim from musteriler where yas>30
--union verileri tekrarsız listeler
...
--intersect
select id as ortak_id from musteriler where sehir='Izmir' --104 105 111
intersect
...
--except operatoru ilk sorgu sonucunun ikinci sorgu sonucundan farkını verir
--except icin sorguların sırası onemlidir
